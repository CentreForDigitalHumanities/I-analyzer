...
def test_alias(test_app, es_alias_client):
    ...
    assert res.get('ianalyzer-test-times_2') is not None

def test_alias_with_clean(test_app, es_alias_client):
    corpus_definition = load_corpus('times')
    indices = es_alias_client.indices.get(index='{}_*'.format(corpus_definition.es_index))
    assert 'ianalyzer-test-times_1' in list(indices.keys())
    alias('times', corpus_definition, True)
    indices = es_alias_client.indices.get(index='{}_*'.format(corpus_definition.es_index))
    assert 'ianalyzer-test-times_1' not in list(indices.keys())
