...
from wordmodels.utils import load_word_models, word_in_corpus_model, transform_query, term_to_vector
...
def test_term_to_vector(test_app, mock_corpus):
    corpus = load_corpus(mock_corpus)
    model = load_word_models(corpus)

    vec1 = term_to_vector('whale', model)
    vec2 = term_to_vector('Whale!', model)

    assert np.all(np.equal(vec1, vec2))
    assert type(vec1) != type(None)

    vec3 = term_to_vector('man', model)

    assert not np.all(np.equal(vec1, vec3))

    novec = term_to_vector('skdfjksdjfkdf', model)
    assert novec == None
