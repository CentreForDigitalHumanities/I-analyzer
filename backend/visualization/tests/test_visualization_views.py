...
@pytest.fixture
def date_term_frequency_body(basic_query, mock_corpus, select_small_mock_corpus):
    return {
...
@pytest.fixture
def aggregate_term_frequency_body(basic_query, mock_corpus, select_small_mock_corpus):
    return {
        'es_query': basic_query,
        'corpus_name': mock_corpus,
        'field_name': 'genre',
...
@pytest.fixture
def ngram_body(basic_query, mock_corpus, select_small_mock_corpus):
    return {
        'es_query': basic_query,
        'corpus_name': mock_corpus,
        'field': 'content',
...
def test_ngrams(transactional_db, admin_client, ngram_body, index_mock_corpus, celery_worker):
    post_response = admin_client.post('/api/visualization/ngram', ngram_body, content_type='application/json')
    assert post_response.status_code == 200

def test_aggregate_term_frequency(transactional_db, admin_client, aggregate_term_frequency_body, index_mock_corpus, celery_worker):
...
def test_date_term_frequency(transactional_db, admin_client, date_term_frequency_body, index_mock_corpus, celery_worker):
