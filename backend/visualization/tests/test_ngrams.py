...
from visualization.tests.mock_corpora.mock_corpus import MockCorpus
...
def test_total_time_interval_no_filter(basic_query, mock_corpus):
...
def test_time_bins(mock_corpus, basic_query):
    # 100 year interval
...
def test_short_interval(mock_corpus, basic_query):
...
def test_absolute_bigrams(indexed_mock_corpus, basic_query):
    ...
    result = ngram.get_ngrams(frequent_query, indexed_mock_corpus, 'content', freq_compensation=False)
...
def test_bigrams_with_quote(indexed_mock_corpus, basic_query):
    cases = [
        {
            'query': '"to hear"',
            'ngrams': [
    ...
    for case in cases:
        ...
        result = ngram.get_ngrams(case_query, indexed_mock_corpus, 'content', freq_compensation=False)
...
def test_number_of_ngrams(indexed_mock_corpus, basic_query):
    # search for a word that occurs a few times
    frequent_query = query.set_query_text(basic_query, 'to')
    ...
    for size in range(1, max_frequency + 2):
        result = ngram.get_ngrams(frequent_query, indexed_mock_corpus, 'content', number_of_ngrams= size)
        series = result['words']
