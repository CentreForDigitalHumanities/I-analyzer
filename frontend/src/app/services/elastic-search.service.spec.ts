...
import { AuthServiceMock, UnauthenticatedMock } from '../../mock-data/auth';
...
describe('ElasticSearchService', () => {
    ...
    describe('with authenticated user', () => {
        ...
        it('should fetch tags', async () => {
            ...
            const result = await response;
            expect(result.documents[0].tags$).toBeTruthy();
        });
    });
        ...
        beforeEach(() => {
            TestBed.configureTestingModule({
                providers: [
                    ElasticSearchService,
                    { provide: TagService, useValue: new TagServiceMock() },
                    { provide: AuthService, useValue: new UnauthenticatedMock()}
                ],
                imports: [ HttpClientTestingModule ]
            });
            service = TestBed.inject(ElasticSearchService);
    ...
    it('should not fetch tags', async () => {
        const queryModel = new QueryModel(mockCorpus);
        const size = 2;
        const response = service.loadResults(queryModel, {from: 0, size, sort: [undefined, 'desc']});
        const searchUrl = `/api/es/${mockCorpus.name}/_search`;
        ...
        httpTestingController.verify();
        const result = await response;
        expect(result.documents[0].tags$).toBeFalsy();
    });
});
