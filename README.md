...
npm install
```
4. Create the file `api/ianalyzer/config.py` (see `api/ianalyzer/default_config.py`). `ianalyzer/config.py` is included in .gitignore and thus not cloned to your machine. The variable `CORPORA` specifies which corpora are available, and the path of the corpus module.
